...
- **`logoutUri` not set — local logout:** the provider (SSO) session lives on, so on logout the plugin sets a session cookie (no max-age) that keeps the login page showing (no automatic re-login) until the browser closes or the user signs in again. Works even without provider end-session support, but the user has to click to sign back in.
